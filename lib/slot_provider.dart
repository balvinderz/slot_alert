...
class SlotProvider extends ChangeNotifier {
  ...
  Future<void> getDistricts() async {
    try {
      String baseUrl =
          "https://cdn-api.co-vin.in/api/v2/admin/location/districts/${selectedState.stateId}";
      districts.clear();
  ...
  Future<void> startSlotAlerter() async {
    ...
    if (Platform.isMacOS) {
      audioPlayer = AudioPlayer();
      await audioPlayer.setAsset("assets/test.mp3");

    } else {
      media2 = await Media.asset('assets/test.mp3');
      player = new Player(
        id: 69420,
        videoWidth: 0,
        videoHeight: 0,
    ...
    while (true) {
      if (started) {
        ...
        if (searchType == SearchType.district) {
          baseUrl =
              "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByDistrict?district_id=${selectedDistrict.districtId}&date=$todaysDate}";
        } else
          baseUrl =
        ...
        try {
          ...
          if (centers.length == 0 && searchType == SearchType.pinCode) {
            scaffoldKey.currentState.showSnackBar(SnackBar(
              content: Text(
                "No Centers found for this Pincode :*(",
          ...
          scaffoldKey.currentState.showSnackBar(SnackBar(
            content: Text(
              "Some Error Occured :( .  Waiting for $waitTime seconds",
              style: TextStyle(color: Colors.white),
            ),
            backgroundColor: Colors.red,
          ));
